...
window.SecurityCore = {
    ...
    /**
     * منع هجمات CSRF - تم التحديث لاستخدام نظام التوكن الحقيقي في السيرفر
     */
    preventCSRF: function() {
        console.log('🛡️ تم تفعيل حماية CSRF عبر السيرفر (Real Token Validation)');
        // تم نقل التحقق بالكامل إلى السيرفر عبر middleware csurf
        // العميل (api-client.js) يقوم الآن بإرسال X-CSRF-Token تلقائياً
    },
    ...
    addSecurityHeaders: function() {
        try {
            // استخدام سياسة CSP محسنة
            const csp = document.createElement('meta');
            csp.httpEquiv = 'Content-Security-Policy';
            csp.content = "default-src 'self'; " +
                          "script-src 'self' 'unsafe-inline' https://www.gstatic.com https://cdnjs.cloudflare.com https://*.firebaseio.com; " +
