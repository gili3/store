...
(function() {
    ...
    async function saveUserToFirestore(user, additionalData = {}) {
        ...
        try {
            ...
            // التحقق من وجود صلاحيات أدمن في التوكن أو البيانات الإضافية
    ...
    async function signInWithGoogle() {
        ...
        try {
            ...
            // إنشاء جلسة خادم فوراً لضمان صلاحيات الوصول
            try {
                ...
                console.error('[Auth] فشل إنشاء جلسة الخادم:', sessionErr);
            }
    ...
    async function checkAuthState() {
        ...
        window.firebaseModules.onAuthStateChanged(auth, (user) => {
            ...
            const isIndexPage = currentPath === '/' || currentPath === '/index';
            ...
            if (isLoginPage) {
                ...
                hideLoader();
            } else if (!isIndexPage && !currentPath.includes('404') && 
                       !currentPath.includes('about') && !currentPath.includes('contact')) {
