...
/**
 * تحديث عرض السلة بشكل إجباري (نسخة متزامنة مع Firebase)
 */
async function forceUpdateCartDisplay() {
    ...
    // ✅ تحديث الملخص (جلب البيانات من Firebase فقط)
    await updateCartSummaryEnhanced(items, currency);
    updateCartCount();
...
/**
 * جلب إعدادات التوصيل من Firebase فقط
 */
async function getShippingSettings() {
    try {
        ...
        if (!db || !window.firebaseModules) {
            ...
            return { shippingCost: 0, freeShippingLimit: 0 };
        }
        
        const settingsRef = window.firebaseModules.doc(db, 'settings', 'store');
        ...
        if (settingsSnap.exists()) {
            ...
            return {
                ...
                freeShippingLimit: parseFloat(data.freeShippingLimit) || 0
            };
        }
        
        console.warn('⚠️ وثيقة الإعدادات غير موجودة في Firebase');
        return { shippingCost: 0, freeShippingLimit: 0 };
    } catch (error) {
        console.error('❌ خطأ في جلب إعدادات التوصيل من Firebase:', error);
        return { shippingCost: 0, freeShippingLimit: 0 };
    }
}

/**
 * تحديث ملخص السلة المحسن (جلب البيانات من Firebase فقط)
 */
async function updateCartSummaryEnhanced(items, currency) {
    ...
    // ✅ جلب إعدادات التوصيل من Firebase فقط (بدون AppState)
    const { shippingCost: shippingCostValue, freeShippingLimit } = await getShippingSettings();
    
    console.log('📦 [Firebase] إعدادات التوصيل:', { shippingCostValue, freeShippingLimit });
    
    const subtotal = items.reduce((total, item) => {
        return total + (parseFloat(item.price) || 0) * (parseInt(item.quantity) || 1);
    }, 0);
    
    console.log('💰 حساب التوصيل:', { subtotal, freeShippingLimit, shippingCostValue });
    
    // حساب الشحن النهائي
    let finalShipping = shippingCostValue;
    
    // ✅ التوصيل مجاني فقط إذا كان هناك حد محدد أكبر من 0 والمجموع يساوي أو يتجاوز الحد
    if (freeShippingLimit > 0 && subtotal >= freeShippingLimit) {
        finalShipping = 0;
        console.log('🎉 توصيل مجاني - تم تجاوز الحد');
    } else if (freeShippingLimit <= 0) {
        // إذا كان الحد 0 أو أقل، لا يوجد توصيل مجاني أبداً
        finalShipping = shippingCostValue;
        console.log('💰 لا يوجد حد للتوصيل المجاني، التوصيل: ' + shippingCostValue);
    } else {
        console.log(`💰 لم يتم تجاوز الحد: ${subtotal} < ${freeShippingLimit}`);
    }
    ...
    // ✅ تحديث ملاحظة التوصيل (تختفي عند الوصول للحد)
    if (shippingNote) {
        // تظهر فقط إذا: يوجد شحن + حد محدد + لم نصل للحد بعد
        if (finalShipping > 0 && freeShippingLimit > 0 && subtotal < freeShippingLimit) {
            const remaining = freeShippingLimit - subtotal;
            shippingNote.innerHTML = `<i class="fas fa-truck"></i> أضف ${formatNumber(remaining)} ${currency} أخرى للتوصيل المجاني`;
...
console.log('🚀 cart-system.js (الإصدار النهائي - جلب من Firebase فقط) جاهز للعمل!');