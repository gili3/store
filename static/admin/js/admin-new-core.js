 ...
 * تم الإصلاح: 2026-05-31 | تم التحديث: دعم جلسة الخادم
 */
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // التحقق من تحميل الملفات الأساسية
            ...
            if (user) {
                ...
                try {
                    ...
                    if (userDoc.exists()) {
                        ...
                        if (!isAdmin) {
                            console.warn('⚠️ [DEBUG MODE] تم تجاوز منع المستخدم غير المسؤول');
                            // تم التعطيل للتجربة
                            /*
                        ...
                        console.log('✅ مستخدم مسؤول، جاري إنشاء جلسة الخادم...');
                        
                        // إنشاء جلسة على الخادم باستخدام ID Token الحالي
                        try {
                            ...
                            if (currentUser) {
                                ...
                                if (!response.ok) {
                                    ...
                                    throw new Error(errorData.error || 'فشل إنشاء الجلسة');
                                }
                                console.log('✅ جلسة الخادم تم إنشاؤها بنجاح');
                            } else {
                                console.warn('⚠️ لا يوجد مستخدم حالي لإنشاء الجلسة');
                            }
                        } catch (err) {
                            console.error('❌ فشل إنشاء جلسة الخادم:', err);
                            if (window.adminUtils && window.adminUtils.showToast) {
                                window.adminUtils.showToast('حدث خطأ في المصادقة مع الخادم، يرجى تحديث الصفحة', 'error');
                            }
                        }
                        ...
                        console.warn('⚠️ [DEBUG MODE] مستند المستخدم غير موجود في Firestore، جاري المتابعة للتجربة');
                        // window.location.href = '/';
                ...
                console.warn('⚠️ [DEBUG MODE] لا يوجد مستخدم مسجل، تم تجاوز التوجيه لصفحة الدخول');
                // window.location.href = '/login';
            }
        });
...
console.log('✅ تم تحميل admin-new-core.js بنجاح (نسخة مصلحة مع دعم جلسة الخادم)');